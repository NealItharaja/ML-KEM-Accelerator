...
module coeff_ram_tb;
    ...
    initial begin
        ...
        for(i = 0; i < 4; i = i + 1) begin
            ...
            $display("WRITE (%0d,%0d) <= (%0d,%0d)", wr_addr_a, wr_addr_b, wr_data_a, wr_data_b);
        end
        ...
        for(i = 0; i < 4; i = i + 1) begin
            ...
            $display("READ (%0d,%0d) -> (%0d,%0d)", rd_addr_a, rd_addr_b, rd_data_a, rd_data_b);
        ...
        if (rd_data_a != 500) begin
            errors = errors + 1;
            $display("FAIL: (0,2) A");
        end
        if (rd_data_b != 502) begin
            errors = errors + 1;
            $display("FAIL: (0,2) B");
        end
        ...
        if (rd_data_a != 501) begin
            errors = errors + 1;
            $display("FAIL: (1,3) A");
        end
        if (rd_data_b != 503) begin
            errors = errors + 1;
            $display("FAIL: (1,3) B");
        end
